...
import { getPoetByIdFn, updatePoetFn } from '@/lib/poets-api'

export const Route = createFileRoute('/poets/$poetId/edit')({
  component: EditPoetPage,
})

function EditPoetPage() {
  const { poetId } = Route.useParams()
  const { user, loading, getToken } = useAuth()
  const [poet, setPoet] = useState<Poet | null>(null)
  const [title, setTitle] = useState('')
  const [content, setContent] = useState<any>(null)
  const [saving, setSaving] = useState(false)
  const [loadingPoet, setLoadingPoet] = useState(true)
  ...
  useEffect(() => {
    const loadPoet = async () => {
      ...
      try {
        const data = await getPoetByIdFn({ data: { token, poetId: Number(poetId) } })
        setPoet(data)
        setTitle(data.title)
  ...
  if (!poet) {
    return (
      <div className="min-h-screen bg-[#F5F1E8] dark:bg-[#2C2416] flex items-center justify-center">
        <div className="text-red-700 dark:text-red-400 text-xl">Poem not found</div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-[#F5F1E8] dark:bg-[#2C2416]">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-bold text-neutral-900 dark:text-neutral-50 font-serif mb-2">Edit Poem</h1>
          <p className="text-neutral-600 dark:text-neutral-400">Refine your masterpiece</p>
        </div>
